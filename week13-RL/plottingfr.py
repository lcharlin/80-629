...
def plot_value_function(V, title="Value Function"):
    ...
    def plot_surface(X, Y, Z, title):
        ...
        surf = ax.plot_surface(X, Y, Z, rstride=1, cstride=1,
                               cmap=matplotlib.cm.coolwarm, vmin=-1.0, vmax=1.0)
        ax.set_xlabel('Score du joueur')
        ax.set_ylabel('Carte visible de la banque')
...
def plot_policy(V, title="Politique"):
    ...
    def plot_surface(X, Y, Z, title):
        fig = plt.figure(figsize=(20, 10))
        ax = fig.add_subplot(111)#, projection='3d')
        surf = ax.scatter(Y, X, c=Z, 
                                cmap=matplotlib.cm.coolwarm) #, rstride=1, cstride=1,
                               #cmap=matplotlib.cm.coolwarm, vmin=-1.0, vmax=1.0)
        ax.set_ylabel('Score du joueur')
        ax.set_xlabel('Carte visible de la banque')
        #ax.set_zlabel('Valeur')
        ax.set_title(title)
        #ax.view_init(ax.elev, -120)
        #fig.colorbar(surf)
